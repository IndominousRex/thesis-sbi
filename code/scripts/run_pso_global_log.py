#!/usr/bin/env python
"""
PSO optimization of vehicle model parameters (log scale version).

This script is identical to run_pso_global.py, but optimizes selected parameters in log-space.
"""
import sys
import os
import argparse
...
# --- Utility: log/exp transform for selected params ---
...
# --- Bounds in log scale for selected params ---
log_global_bounds = []
for name in GLOBAL_PARAMS:
    low, high = GLOBAL_BOUNDS[name]
    if name in LOG_SCALE_PARAMS:
        log_global_bounds.append((np.log(low), np.log(high)))
    else:
        log_global_bounds.append((low, high))


# --- The rest of the code is identical, except decode_params and objective ---
...
def main():
    ...
    for i, data in enumerate(prepared_data):
        ...
        mse = float(np.mean(OBS_WEIGHTS * (y_sim - data["y_real"]) ** 2))
        print(f"  Traj {i+1} [{data['surface']:10s}]: MSE = {mse:.6f}")
