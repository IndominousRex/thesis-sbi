...
class SimformerPosterior:
    ...
    def _init_backward_sde(self, x_o_embedded: Array):
        ...
        def drift_backward(t, x):
            ...
            drift = self.sde.drift(t_actual, x) - self.sde.diffusion(t_actual, x)**2 * score
    ...
    def sample(
        ...
        **kwargs,
        ...
        theta_samples = self._sample_jax(
            num_samples, x_o_embedded, key, self.num_steps
        )
...
class SimformerMethod(BaseMethod):
    ...
    def build(self, input_dim: int, seq_len: int) -> None:
        ...
        self.embedding_net = build_embedding(
            self.cfg, input_dim, seq_len, self.device
        )
        ...
        print(f"[Simformer] Built with {self.theta_dim} params + {self.embedding_dim} embedding = {self.total_nodes} nodes")
    ...
    def _build_score_model(self):
        ...
        def model(t: Array, x: Array, node_ids: Array, condition_mask: Array):
            ...
            embedding_net_id = hk.Embed(
                total_nodes, token_dim,
                w_init=hk.initializers.RandomNormal(stddev=3.0)
            )
    ...
    def train(
        ...
        key = jrandom.PRNGKey(self.cfg.random_seed)
        
        # Initialize parameters
        dummy_condition = jnp.zeros(self.total_nodes, dtype=jnp.bool_)
        self.params = init_fn(
            key, jnp.ones(joint_data_jax.shape[0]),
            joint_data_jax, node_ids, dummy_condition
        )
        self.model_fn = model_fn
        ...
        def loss_fn(params, key, batch_size=self.batch_size):
            ...
            # Sample batch
            indices = jrandom.randint(keys[0], (batch_size,), 0, joint_data_jax.shape[0])
            batch_xs = joint_data_jax[indices]
            
            # Random times
            times = jrandom.uniform(keys[1], (batch_size, 1, 1), minval=self.T_min, maxval=self.T_max)
            
            # Random condition mask (for learning all conditionals)
            condition_mask = jrandom.bernoulli(keys[2], 0.333, shape=(batch_size, self.total_nodes))
            all_conditioned = jnp.all(condition_mask, axis=-1, keepdims=True)
            condition_mask = condition_mask * ~all_conditioned  # Avoid conditioning on everything
            condition_mask = condition_mask[..., None]
            
            loss = denoising_score_matching_loss(
                params, keys[3], times, batch_xs, condition_mask,
                model_fn=model_fn,
        ...
        losses = []
        key = jrandom.PRNGKey(self.cfg.train_seed or 0)
        ...
        print(f"[Simformer] Training complete in {train_time:.1f}s, final loss = {self._training_summary['final_loss']:.4f}")
